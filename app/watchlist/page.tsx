...
export default function WatchlistPage() {
  ...
  const getPriceColor = (change: number) => {
    if (change > 0) return 'text-red-500'; // 上漲用紅色
    if (change < 0) return 'text-green-500'; // 下跌用綠色
    return 'text-slate-400';
  };

  const getBackgroundColor = (change: number) => {
    if (change > 0) return 'bg-red-900/20 border-l-red-500'; // 上漲背景
    if (change < 0) return 'bg-green-900/20 border-l-green-500'; // 下跌背景
              ...
              {watchlist.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* 備註 */}
                    {item.notes && (
                      <div className="text-xs text-slate-300 mb-2 p-2 bg-slate-700/50 rounded">
                        {item.notes}
                      </div>
                    )}
